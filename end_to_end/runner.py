...
PRETRAINED_MLP_PATH = None # '/data/verifiedanivray/mlp_net_weights.pth'
POSTRAINED_MODEL_PATH = '/data/verifiedanivray/embedding_net_weights.pth'
POSTRAINED_MLP_PATH = '/data/verifiedanivray/mlp_net_weights.pth'
...
# CREATE TRIPLET NET
endtoend_net = EndtoEndNet(embedder)
# Freeze the embedding net
endtoend_net.embedding_net.requires_grad_(False)

# load saved weights for mlp!
# embedder.head.load_state_dict(torch.load(PRETRAINED_MODEL_PATH))

# TRAIN

learning_rate = 0.001 # ResNet-18 & 34
scheduler=None # not needed for Adam
optimizer = optim.Adam(endtoend_net.parameters(), lr=learning_rate)

log += 'learning rate = {}'.format(learning_rate)

best_val_epoch, best_val_loss = 0, 0

best_val_epoch, best_val_loss = fit(train_loader=train_dataloader, val_loader=val_dataloader, model=endtoend_net, \
    loss_fn=nn.BCEWithLogitsLoss(), optimizer=optimizer, scheduler=scheduler, \
    n_epochs=100, cuda='cuda:1', log_interval=300, metrics=[], start_epoch=0, early_stopping_interval=5, \
...
# SAVE MLP
torch.save(endtoend_net.head.state_dict(), POSTRAINED_MLP_PATH)

from datetime import datetime
datetime_str = datetime.now().strftime("%Y-%m-%d_%H:%M:%S")
with open('/data/verifiedanivray/results/results_{}.txt'.format(datetime_str), 'w') as fout:
    fout.write(log + '\n')
torch.save(embedder.state_dict(), '/data/verifiedanivray/results/weights_{}.pth'.format(datetime_str))
