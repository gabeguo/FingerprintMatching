...
def fit(train_loader, val_loader, model, loss_fn, optimizer, scheduler, n_epochs, cuda, log_interval, metrics=[],
    ...
    for epoch in range(start_epoch, n_epochs):
        ...
        if len(past_val_losses) > early_stopping_interval and val_loss > sum(past_val_losses[-early_stopping_interval:]) / len(past_val_losses[-early_stopping_interval:]):
            ...
            with open('/data/verifiedanivray/results/train_res_{}.txt'.format(datetime_str), 'w') as fout:
...
def train_epoch(train_loader, model, loss_fn, optimizer, cuda, log_interval, metrics, \
    ...
    for batch_idx, (data, target, filepaths) in enumerate(train_loader):
        ...
        if random() < prior:
            data = (data[0], data[1])
            target = torch.ones((data[0].size()[0], 1)).cuda(device=cuda)
        else:
            data = (data[0], data[2])
            target = torch.zeros((data[0].size()[0], 1)).cuda(device=cuda)
        assert(len(data) == 2)
        outputs = model(*data)
