...
-> matrix f2f_dist of finger by finger distances where f2f_dist[i][j][k] = 
    list of distances between FGRP (i, j) for matching status k (SAME_PERSON or DIFF_PERSON)
"""
def run_test_loop(test_dataloader, model, cuda, num_anchors, num_pos, num_neg, pos_indices):
    ...
    for i in range(len(test_dataloader)):
        ...
        for i_a in range(num_anchors):
            ...
            # embedding_anchor = torch.flatten(model(curr_anchor))
            ...
            for triplet_sameness_idx, sameness_code, num_samples, curr_dists \
                ...
                if (not isPos) != sameness_code:
                    continue
                print("Positive" if sameness_code == 0 else "Negative")
                for i_curr in range(num_samples):
                    curr_sample = test_images[triplet_sameness_idx][i_curr].to(cuda)
                    probability = torch.sigmoid(model(curr_anchor, curr_sample)).item()
                    print("Probability = ", probability)
                    correctness = int(abs(sameness_code - probability) > 0.5)
                    print("Correct? ", correctness)
                    # embedding_curr = torch.flatten(model(curr_sample))
                    # assert len(embedding_curr.size()) == 1 and embedding_curr.size(dim=0) == 512
                    curr_dists.append(correctness)
...
def acalculate_baseline(prior, alpha):
    A = prior
...
def main(the_data_folder, weights_path, cuda, output_dir, num_anchors, num_pos, num_neg, \
    ...
    model = EndtoEndNet(embedding_net=embedder)
    model.head.load_state_dict(torch.load(weights_path))
    model.eval()
    model.to(cuda)
    ...
    pos_indices = rand.sample(all_indices, num_pos_samples)
    pos_indices.sort()
    ...
    _01_dist, _02_dist = run_test_loop(test_dataloader=test_dataloader, model=model, cuda=cuda, \
        num_anchors=num_anchors, num_pos=num_pos, num_neg=num_neg, pos_indices=pos_indices)

    combined_results = _01_dist + _02_dist
    acc = ( sum(combined_results) / len(combined_results) ) * 100
    print("Accuracy = ", acc)
