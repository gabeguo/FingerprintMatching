...
def fit(train_loader, val_loader, model, train_loss_fn, val_loss_fn, optimizer, scheduler, n_epochs, cuda, log_interval, metrics=[],
    ...
    for epoch in range(start_epoch, n_epochs):
        ...
        # Train stage
        train_loss, metrics = train_epoch(train_loader, model, train_loss_fn, optimizer, cuda, log_interval, metrics)
        ...
        val_loss, metrics = test_epoch(val_loader, model, val_loss_fn, cuda, metrics)
...
def train_epoch(train_loader, model, loss_fn, optimizer, cuda, log_interval, metrics):
    ...
    for batch_idx, (data, target, filepaths) in enumerate(train_loader):
        ...
        assert type(loss_fn) == OnlineTripletLoss

        loss_inputs = list(loss_inputs)
        loss_inputs[0] = loss_inputs[0].reshape(loss_inputs[0].size(dim=0), loss_inputs[0].size(dim=1))
